...
public class StructureSelector : BaseSelector {//attached to each building as an invisible 2dtoolkit button
	...
	{			
		...
		for (int i = 0; i < ResourceGenerator.InstanceResourceGenerator.basicEconomyValues.Length; i++) 
		{
			if (ResourceGenerator.InstanceResourceGenerator.basicEconomyValues [i].StructureType == structureType) 
			{					
				CopyBasicValues (ResourceGenerator.InstanceResourceGenerator.basicEconomyValues [i]);
				break;
			}							
		}
	}
	...
	private IEnumerator CalculateElapsedProduction(int elapsedTime)//private IEnumerator
	{
		...
		for (int i = 0; i < ResourceGenerator.InstanceResourceGenerator.basicEconomyValues.Length; i++) 
		{
			if (ResourceGenerator.InstanceResourceGenerator.basicEconomyValues [i].structureIndex == structureIndex) 
			{	
