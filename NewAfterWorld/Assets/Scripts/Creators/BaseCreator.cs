...
public class BaseCreator : MonoBehaviour {
	...
	public Component stats, soundFX, transData, cameraController, relay, statusMsg, menuMain/*, resourceGenerator*/;//protected
	...
	protected void Update () {
		if (MovingPad.activeSelf) 
		{
			...
			if (Input.GetMouseButton (0) && finishedMoving) 
			{
				OK ();
				/*
	...
	*/
		...
		if (canBuild) 
		{
			...
			{
				((Stats)stats).level++;
				//Kludge by Qasim for updating town hall level and restricting buidlings. Referred to UA for fixing
				if(((Stats)stats).townHallLevel < ((Stats)stats).level)
					((Stats)stats).townHallLevel = ((Stats)stats).level;
