...
public class SaveLoadBase : MonoBehaviour {
	...
	*/
	...
	private IEnumerator LateUpdateProduction(int index, int timeSpan)
	{
		...
		//EconomyBuilding lastEconomyBuilding = (ResourceGenerator.InstanceResourceGenerator.existingEconomyBuildings [index];
		(ResourceGenerator.InstanceResourceGenerator.FastPaceProductionIndividual (index, timeSpan);
	}
	*/
	...
	private IEnumerator CalculateElapsedTime()//protected void
	{
		...
		if(!isBattleMap)
		ResourceGenerator.InstanceResourceGenerator.FastPaceEconomyAll (elapsedTime);
		...
		if(!isBattleMap)
		{
			...
			//(ResourceGenerator.InstanceResourceGenerator.InitializeEconomy();
	...
	{				
		...
		{
			for (int i = 0; i < ResourceGenerator.InstanceResourceGenerator.existingEconomyBuildings.Count; i++) 
			{
				if ((ResourceGenerator.InstanceResourceGenerator.existingEconomyBuildings [i]).structureIndex == eB.structureIndex) 
				{
					(ResourceGenerator.InstanceResourceGenerator.existingEconomyBuildings [i]).ModifyGoldAmount (eB.storedGold);
					(ResourceGenerator.InstanceResourceGenerator.existingEconomyBuildings [i]).ModifyManaAmount (eB.storedMana);
					break;
				}
			}
		}
